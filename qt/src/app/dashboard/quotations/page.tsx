...
// jsPDF is used to generate the downloadable quotation PDF client-side.
// Install it in your project with:  npm install jspdf
// It's dynamically imported inside downloadQuotationPDF so it never
// bloats the initial page bundle.
...
export default function QuotationsPage() {
  ...
  const filteredQuotations = useMemo(() => {
    ...
    return quotations.filter(
      (quotation) =>
        quotation.id.toLowerCase().includes(searchValue) ||
        quotation.customer.toLowerCase().includes(searchValue) ||
        quotation.email.toLowerCase().includes(searchValue) ||
        quotation.status.toLowerCase().includes(searchValue)
    );
  }, [quotations, search]);
  ...
  const createQuotation = (e: React.FormEvent) => {
    ...
    if (!form.customer.trim() || !form.email.trim() || form.total === "") {
  ...
  const deleteQuotation = (id: string) => {
    ...
    saveQuotations(quotations.filter((quotation) => quotation.id !== id));
  ...
  const downloadQuotationPDF = async (quotation: Quotation) => {
    try {
      ...
      doc.setFontSize(11);
      doc.text("RenderWonders", marginX, 40);

      doc.setFontSize(20);
      doc.text("Quotation", marginX, 68);

      doc.setFontSize(11);
      doc.text(quotation.id, pageWidth - marginX, 68, { align: "right" });

      // Body
      doc.setTextColor(107, 114, 128); // gray-500
      doc.setFontSize(9);
      doc.text("BILLED TO", marginX, 140);
      doc.text("DATE", pageWidth - marginX, 140, { align: "right" });

      doc.setTextColor(17, 24, 39);
      doc.setFontSize(12);
      doc.text(quotation.customer, marginX, 158);
      doc.setFontSize(10);
      doc.setTextColor(75, 85, 99);
      doc.text(quotation.email, marginX, 174);

      doc.setFontSize(12);
      doc.setTextColor(17, 24, 39);
      doc.text(quotation.date, pageWidth - marginX, 158, { align: "right" });

      doc.setTextColor(107, 114, 128);
      doc.setFontSize(9);
      doc.text("STATUS", pageWidth - marginX, 190, { align: "right" });
      doc.setTextColor(17, 24, 39);
      doc.setFontSize(12);
      doc.text(quotation.status, pageWidth - marginX, 208, {
        align: "right",
      });

      // Divider
      doc.setDrawColor(229, 231, 235); // gray-200
      doc.line(marginX, 236, pageWidth - marginX, 236);

      // Total block
      doc.setTextColor(107, 114, 128);
      doc.setFontSize(10);
      doc.text("TOTAL AMOUNT", marginX, 280);
      ...
      doc.line(marginX, 700, pageWidth - marginX, 700);
      ...
      console.error(err);
      alert(
        "Couldn't generate the PDF. Make sure the 'jspdf' package is installed (npm install jspdf)."
      );
    } finally {
  ...
  return (
    <div style={{ fontFamily: fontStack, fontWeight: 300 }}>
        ...
        <div className="mb-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {[
            { label: "Total Quotations", value: quotations.length, color: "text-gray-900" },
            ...
            >
              ...
              <p
                className={`mt-2 text-2xl ${stat.color}`}
                style={{ fontWeight: 400 }}
              >
                {stat.value}
              </p>
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[900px] text-left text-sm">
              <thead className="border-b border-gray-200 bg-[#FAFAFB]">
                <tr>
                  <th className="px-6 py-3 font-normal text-gray-500">Quote #</th>
                  <th className="px-6 py-3 font-normal text-gray-500">Customer</th>
                ...
                {filteredQuotations.length === 0 && (
                  <tr>
                    <td colSpan={4} className="px-6 py-16 text-center">
                      ...
                      <p className="mt-3 text-sm text-gray-700" style={{ fontWeight: 500 }}>
                        No quotations found
                      </p>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          ...
          >
            ...
            <form onSubmit={createQuotation} className="space-y-3 p-6">
              <input
                type="text"
                value={form.customer}
              ...
              <select
                value={form.status}
                onChange={(e) =>
                  setForm({ ...form, status: e.target.value as QuotationStatus })
                }
          ...
          >
            <div className="flex items-center justify-between border-b border-gray-200 px-6 py-5">
              <div>
                ...
                <h2 className="mt-1 text-xl text-gray-900" style={{ fontWeight: 400 }}>
            ...
            <div className="space-y-5 p-6">
              <div>
                ...
                <p className="mt-1 text-gray-900" style={{ fontWeight: 500 }}>
                  {selectedQuotation.customer}
                </p>
                <p className="text-sm text-gray-500">{selectedQuotation.email}</p>
              </div>
              ...
              <div className="rounded-2xl bg-[#F5F6F7] p-4">
                ...
                <p className="mt-1 text-2xl text-gray-900" style={{ fontWeight: 400 }}>
